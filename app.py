        ...
        with st.spinner('正在拉取資料並運算中...'):
            try:
                ...
                for index, row in df_stock.iterrows():
                    ...
                    # 🔍 核心功能：檢查有效期限
                    # ==========================================
                    expiry_str = str(row.get('有效期限', '')).strip()
                    expiry_display = "-"
                    if expiry_str:
                        try:
                            # 計算今天到過期日還有幾天
                            ...
                            if days_to_expire < 0:
                                expiry_alerts.append(f"**{product}**：已過期 {abs(days_to_expire)} 天 ❌ (請立即報廢)")
                                expiry_display = "已過期"
                            elif days_to_expire <= 3:
                                expiry_alerts.append(f"**{product}**：剩下 {days_to_expire} 天過期 ⚠️ (請優先使用)")
                                expiry_display = f"剩 {days_to_expire} 天"
                            else:
                                expiry_display = f"剩 {days_to_expire} 天"
                        except:
                            expiry_display = expiry_str # 萬一日斯格式不對，就照原本顯示
                    ...
                    if current_stock <= SAFE_STOCK_LEVEL:
                        needs_alert = True
                        alert_reason = f"數量極低 (剩 {int(current_stock)})"
                    elif days_remaining <= 3:
                        needs_alert = True
                        alert_reason = f"撐不過 3 天"
                    ...
                    report_data.append({
                        ...
                        "期限狀態": expiry_display, # 🌟 表格多加一個期限狀態欄位
                ...
                # ==========================================
                # 畫面呈現區：食安警報 > 庫存警報 > 完整表格 > 圖表
                # ==========================================
                
                # 1. 最優先顯示：食安即期警報
                if expiry_alerts:
                    st.error("🚨 **【食安警報：即期/過期品提醒】** 請廚房優先處理以下食材！")
                    for item in expiry_alerts:
                        st.warning(f"🍱 {item}")

                # 2. 顯示：庫存低警報與 LINE 按鈕
                if alert_items:
                    st.error("🚨 **【庫存過低警報】** 請盡速安排補貨！")
                ...
                # 如果都很安全
                if not alert_items and not expiry_alerts:
                    st.success("✅ 目前所有食材庫存量皆充足，且無即期品！")
